...
from db_helpers import (
    ...
    can_take_test,get_users,get_test_duration,get_retake_db, decrement_retake
)
...
def run_student_mode(): # ✅ Ensures we always have correct Streamlit reference
    ...
    st.markdown(
        ...
        <marquee behavior="scroll" direction="left" scrollamount="5"
            style="color: #F54927; background-color: ; padding: 8px; border-radius: 8px; font-weight: bold;">
            🚀 Warning: Retakes are only enabled by Admins! 🎯 | 📢 Remember to submit your test before auto submit ⏳
        </marquee>
        """,
    ...
    # ===========================================================
    # Show Remaining Retakes BEFORE start test button
    # ===========================================================
    from db_helpers import get_retake_db, decrement_retake

    access_code = student.get("access_code", "").strip()

    if student and selected_subject:
        remaining = get_retake_db(access_code, selected_subject)
        if remaining > 0:
            st.info(f"🔄 You have {remaining} retake(s) left for {selected_subject}.")
        else:
            st.warning(f"⚠️ No retakes left for {selected_subject}. Contact Admin for more.")
    ...
    if st.button("Start Test") and not st.session_state.test_started:
        ...
        # ✅ Block start if no retakes left
        if remaining <= 0:
            st.error("No retake attempts left. Contact Admin.")
            st.stop()

        # ✅ Consume ONE retake attempt
        decrement_retake(access_code, selected_subject)

        # Load questions
        questions = load_questions_db(student.get("class", ""), st.session_state.subject)
        if not questions:
            st.warning("No questions found in database for this class/subject.")
            st.stop()
        ...
        st.session_state.update({
            ...
            "current_q": 0,
            "marked_for_review": set(),
            "test_end_time": datetime.now() + timedelta(minutes=get_test_duration())
        })

        st.success(f"✅ Test started for {selected_subject}. Good luck!")
        st.rerun()
    ...
    if st.session_state.submitted:
        correct, details = calculate_score_db(st.session_state.questions, st.session_state.answers)
        ...
        if not allowed:
            ...
            add_submission_db(
                ...
                subject=st.session_state.subject,
                score=correct,
                answers={f"Q{i + 1}": st.session_state.answers[i] for i in range(len(st.session_state.answers))}
            )
